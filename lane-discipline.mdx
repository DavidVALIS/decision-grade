...
| **Triage** | May indicate issues while work changes | May indicate issues, but cannot clear reliance |
| **Full review** | May be used when deeper review is warranted | Required before consideration |
...
flowchart TD
    ...
    C -->|Decision| E[Full review required]
...
## Triage is not clearance

Fast triage may identify issues while a document is changing. It can be useful in either lane, including work that may later support a consequential decision.

Triage never clears Decision work.

A full review is required before Decision-lane consideration. Completion is still not automatic clearance. The result may allow the use, impose a condition, require more work, or refuse.
...
<CardGroup cols={1}>
  <Card title="1. Unlabeled work reaches a consequential decision" icon="triangle-exclamation">
    ...
    **Control:** Unlabeled material defaults to Volume and may not support Decision-lane reliance without reclassification and a full review.
  </Card>
  ...
  <Card title="2. Every important document is called Decision Grade" icon="mask">
    ...
    **Control:** Reserve Decision Grade for one declared use and an earned allowed determination after full review.
  </Card>

  <Card title="3. Triage becomes the deadline shortcut" icon="clock">
    Triage is treated as clearance because full review takes longer.

    **Control:** Make the resulting action state explicit. Triage can indicate; it cannot clear Decision reliance.
  </Card>
...
- Decision-lane documents that received full review before reliance.
- Triage results later used without full review.
