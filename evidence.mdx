...
<StatRow cols={2}>
  <Stat
    value="90–100%"
    label="Frontier LLM differential-diagnosis failure rate across 21 models and 16,254 responses on staged clinical cases"
    source="Mass General Brigham · JAMA Network Open · April 2026"
  />
  <Stat
    value="49%"
    label="Higher rate at which 11 frontier models affirm users' actions versus humans, including for harmful or illegal actions"
  ...
  <Stat
    value="42 of 53"
    label={'Frontier models that said "walk" when asked to walk or drive 50 m to a car wash, even though the car must be present to be washed'}
...
2. **AI agrees with you more than a real person would, and you'll like it for that.** The sycophancy study showed it. The AI is the friend who always tells you you're right. You'll prefer that friend. You'll trust that friend. You'll come back to that friend. And every time you do, the AI is being trained to be more like that friend, because users prefer it. The natural way you'd check the AI — by asking it "are you sure?" — is the exact thing the AI has been trained to defeat.
...
4. **AI fails on the easy questions in ways that hide on the hard questions.** The car wash test is the cleanest demonstration: AI fixates on the surface ("50 meters is close, walk") and misses the actual rule of the problem ("the car has to come with you"). Real decisions almost always have a hidden rule like that — the regulator who won't approve it, the customer who won't pay for it, the team member who'll quit if it ships. AI is bad at noticing the rule it isn't told about. And because it sounds smart, the person reading the answer often misses that the rule got missed.

Put it all together and you get the trap: AI is a fast, smart-sounding assistant; it agrees with you more than a real colleague would; it makes mistakes that look fine until someone goes and checks; and the mistakes are already landing in courtrooms and clinics and boardrooms.

You'll like working with this assistant. You'll trust this assistant. And every so often — sometimes in a hospital, sometimes in a courtroom, sometimes in a strategy memo — the assistant will hand you a polished version of "walk, the car wash is only 50 meters away," and you won't catch it because nothing in the answer looks wrong.
...
<Note>
  **In short:** Current public evidence does not support a single universal AI error rate. It supports something more important: AI failure is task-dependent, measurable, and already visible in production-adjacent workflows. In constrained source-grounded summarization, hallucination rates can fall to low single digits but reach the low teens for many flagship models. In legal research, specialized tools have produced incorrect information in over 17 to 34 percent of tested queries. In AI search, one journalism benchmark found incorrect source identification in more than 60 percent of tested queries. In staged clinical reasoning, frontier LLMs performed well when complete information was supplied but failed at differential-diagnosis steps in 90 to 100 percent of cases. These results do not prove that every AI-assisted decision is unreliable. They prove that polished AI output cannot be treated as verified analysis without task-specific controls.
</Note>
