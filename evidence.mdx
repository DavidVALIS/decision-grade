...
## A small piece of the measurement, done

The four error modes above are deployment-level. The framework needs them measured continuously, in real organisational settings, with public methodology. That work does not yet exist at scale. But a smaller version of it now does.

The [VALIS Benchmarks](/benchmarks/) ran the same three frontier models (Claude Opus 4.7, GPT-5.5, Gemini 3.1 Pro) against six narrow tests every analyst already does in their head before signing a memo. The hardest one, "notice what is missing from an analysis," is the structural failure mode the evidence above predicts. Here is what happened.

<BenchmarkComparisonChart wedge="boundary_condition_omission" />

Three frontier reasoning models. 500 cases each. None cleared 41 percent macro F1. This is one direct, anchored measurement of the omission failure mode the four-error-mode framework names. The pre-registered manifest was signed and Bitcoin-timestamped before any model was called, so the result cannot be retroactively softened.

[See all six tests →](/benchmarks/)
...
<CardGroup cols={2}>
  ...
  <Card title="Benchmarks" icon="bars" href="/benchmarks/">
    Six pre-registered, Bitcoin-anchored tests against three frontier models. Direct measurement of the structural failure modes this page describes.
  </Card>
