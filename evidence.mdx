...
<StatRow cols={2}>
  <Stat
    value="90–100%"
    label="Off-the-shelf frontier LLM differential-diagnosis failure rate across 21 models and 16,254 responses. Reasoning-optimized models including Grok 4, GPT-5, and Claude 4.5 Opus failed at the same rate."
    source="Mass General Brigham · JAMA Network Open · April 2026"
  />
  <Stat
    value="4.85×"
    label="More likely than human experts that LLM summaries of scientific research contain overly broad generalizations, across 4,900 summaries and 10 LLMs (95% CI 3.06 to 7.70, p < 0.001). Newer models performed worse than older ones."
    source="Peters and Chin-Yee · Royal Society Open Science · April 2025"
  />
  <Stat
    value="1,455"
    label="Legal decisions worldwide where AI-generated content (fabricated citations, false quotes, misrepresented authorities) was addressed by a court"
    source="Charlotin AI Hallucination Cases Database · 17 May 2026"
  />
  <Stat
    value="0.59 AUC"
    label="Reviewer accuracy distinguishing correct from incorrect AI answers given default explanations, barely above the 0.50 random-chance line. The model's own internal confidence carries AUC 0.75 to 0.78."
...
1. **AI is fluent at the easy part of thinking, and bad at the hard part.** The medical study showed it. The AIs could name a diagnosis when handed all the clues. They couldn't start the puzzle from a blank page. But the start of the puzzle is where the real work happens. AI is great at finishing your sentence. It's bad at being the one who started the sentence in the first place.

2. **AI strips out the limits and exceptions that matter.** Peters and Chin-Yee showed it. When AI summarizes scientific research, it removes the boundary conditions (the qualifiers like "this only worked in mice," "this only held in patients over 60," "this only applied during the test window") far more often than human experts do. Five times more often, in fact. That is exactly the failure mode that turns "the pilot worked in one segment" into "the strategy works." The facts the AI cites are usually accurate. The frame around the facts is wrong, and the wrong frame is what gets quoted in the next meeting.

3. **The cost is real and it's already landing.** The 1,455 court cases aren't theoretical. They're dated. They're tagged by country. They're growing by five or six new ones a day. Real lawyers got fined real money. Real people had their real cases damaged. We're not warning about a thing that might happen. We're counting the thing that is happening.

4. **The person checking the AI's answer can't reliably tell if it's right.** Steyvers and colleagues showed it. They gave 301 people the AI's answers, asked them to spot the wrong ones, and tracked how well they did. People distinguished correct from incorrect answers barely better than flipping a coin. The model's own internal sense of confidence carried much more information than the reader could extract from the explanation. Longer, more elaborate explanations made readers *more* confident in the AI without making them any better at catching errors. The natural assumption that "a senior person will catch it" is the assumption the data refuses to support.

**Put it all together and you get the trap:** AI is a fast, smart-sounding assistant; its summaries leave out the limits and exceptions that make the difference; the person reading the output can't reliably catch when it's wrong; and the mistakes are already landing in courtrooms and clinics and boardrooms.

You'll like working with this assistant. You'll trust this assistant. And every so often (sometimes in a hospital, sometimes in a courtroom, sometimes in a strategy memo) the assistant will hand you a polished answer whose underlying frame is broken, and you won't catch it because nothing in the answer looks wrong.

The fix isn't "stop using AI." That isn't going to happen. The fix is: don't let an AI's answer count as a decision until something independent has checked it. You wouldn't let your friend file your taxes without you double-checking. Don't let the AI either.

<Note>
  **In short:** Current public evidence does not support a single universal AI error rate. It supports something more important: AI failure is task-dependent, measurable, and already visible in production-adjacent workflows. These results do not prove that every AI-assisted decision is unreliable. They prove that polished AI output cannot be treated as verified analysis without task-specific controls.
</Note>
...
The framework's actual target is sharper than "AI hallucinates." It is: AI-augmented analytical work fails on the relationship between claim, evidence, assumption, comparison, time period, and recommendation, even when individual facts and citations are correct. Five new studies now measure that failure directly in adjacent analytical domains (policy reports, SEC filings, scientific summarization, human calibration on AI output, and the trust / critical-thinking inverse-correlation). They are presented in the [Direct evidence](#direct-evidence-ai-fails-at-analysis-not-just-at-facts) section.

What is still not directly measured is the rate of structural-reasoning failure in *deployed executive decision workflows*: AI-generated board memos, M&A diligence, regulatory submissions, strategy decks. The five direct sources are the closest approach. The gap that remains is the [closing section](#what-is-not-yet-measured).
...
The seven anchor sources above establish that AI fails measurably across grounded summarization, citation retrieval, legal research, clinical reasoning, court filings, and frontier safety controls. That evidence is real but adjacent. The framework's actual target is sharper: AI-augmented analytical work fails on the *relationship* between claim, evidence, assumption, comparison, time period, and recommendation, even when individual facts and citations are correct.

Five recent studies measure that failure directly. They are not yet a single unified benchmark of executive decision memos. They are the closest existing public measurement of structural-reasoning failure in analytical workflows, and they shift the framework from "this gap is asserted" to "this gap is partly observed."
...
<AccordionGroup>
  ...
  <Accordion title="Reasoning settings were disabled for toggleable models in the MGB study, and newer reasoning models post-date the paper">
    The Mass General Brigham paper tested architecturally reasoning-optimized models (Grok 4, GPT-5, Claude 4.5 Opus, Claude 3.7 Sonnet, DeepSeek R1, Gemini 2.5 Pro, Gemini 3.0 Pro, Gemini 3.0 Flash, GPT-o1 series, GPT-o3-Mini) in their reasoning configuration. For models with a *toggleable* reasoning mode (GPT-5's reasoning_effort dial, Claude's extended-thinking switch, Gemini Flash Thinking), the toggle was disabled to assess base deployment behavior. The 90 to 100 percent differential-diagnosis failure rate held across both groups. Models released after the paper's submission window (GPT-5.5 Pro, Claude Opus 4.7, Gemini 3.1 Pro) were not tested. The framework's prediction is that the benchmark-vs-deployment decoupling will persist on those models; the data is not in yet.
  </Accordion>
</AccordionGroup>
